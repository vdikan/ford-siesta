!TODO find out fom of the formula  \(<\phi_{\mu}| exp^( iexpikr * i * \vec{k} \cdot \vec{r} ) |\phi_{\nu}>\)
! 
...
!!
!!###Output
!! `complex(dp) [[m_planewavematrixvar(module):delkmat]](nvmax)`:
...
!  Modules
    ...
    integer, intent(in) :: iexpikr
      !! Prefactor of the dot product between the
      !! the k-vector and the position-vector in exponent.  
      !! It might be +1 or -1
    integer, intent(in) :: no
      !! Number of basis orbitals
    integer, intent(in) :: np
      !! Number of columns in `C` (local)
    integer, intent(in) :: nvmax
      !! First dimension of `listV` and `Vs`, and max
      !! number of nonzero elements in any row of `[[m_planewavematrixvar(module):delkmat]]`
    integer :: nuo, nuotot
    integer, intent(in) :: iaorb(*)
      !! Pointer to atom to which orbital belongs
    integer, intent(in) :: iphorb(*)
      !! Orbital index within each atom
    integer, intent(in) :: isa(*)
      !! Species index of all atoms
    integer, intent(in) :: numVs(nuo)
      !! Number of non-zero elements in a row of `[[m_planewavematrixvar(module):delkmat]]`
    integer, intent(in) :: listVsptr(nuo)
      !! Pointer to the start of rows in `listVs`
    integer, intent(in) :: listVs(nvmax)
      !! List of non-zero elements of `[[m_planewavematrixvar(module):delkmat]]`
