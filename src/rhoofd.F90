...
!! author: P.Ordejon and J.M.Soler
!! date: May 1995
!! license: GNU GPL
!!
...
!! memory. Modified by J.D.Gale, November'99
  ...
  integer, intent(in) :: maxnd
    !! First dimension of `listd` and `Dscf`, and
  ...
  integer, intent(in) :: listdptr(nuo)
    !! Pointer to start of rows in `listd`
  integer, intent(in) :: listd(maxnd)
    !! List of nonzero elements in each row of `Dscf`
  integer, intent(in) :: nspin
    !! Number of spin components
  integer, intent(in) :: nuo
    !! Number of orbitals in unit cell locally
  integer, intent(in) :: nuotot
    !! Number of orbitals in unit cell in total
