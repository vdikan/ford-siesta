...
!             "nl" family
         integer                         ::  n_orbnl=0
          !! num of nl orbs
         integer                         ::  lmax_basis=0
          !! basis l cutoff
         integer, dimension(maxn_orbnl)  ::  orbnl_l
          !! l of each nl orb
         integer, dimension(maxn_orbnl)  ::  orbnl_n
          !! n of each nl orb
         integer, dimension(maxn_orbnl)  ::  orbnl_z
          !! z of each nl orb
         logical, dimension(maxn_orbnl)  ::  orbnl_ispol
          !! is it a pol. orb?
         real(dp), dimension(maxn_orbnl)  ::  orbnl_pop
          !! population of nl orb (total of 2l+1 components)
...
!             1 to the total number of projectors at that l.
         ...
         integer                         ::  n_pjnl=0
          !! num of "nl" projs
         integer                         ::  lmax_projs=0
          !! l cutoff for projs
         integer, dimension(maxn_pjnl)   ::  pjnl_l
          !! l of each nl proj
         real(dp), dimension(maxn_pjnl)  ::  pjnl_j
          !! j of each nl proj
         integer, dimension(maxn_pjnl)   ::  pjnl_n
          !! n of each nl proj
         real(dp), dimension(maxn_pjnl)  ::  pjnl_ekb
          !! energy of
...
!        a further dereference)
         ...
         real(dp), dimension(maxnorbs)   ::  orb_pop
          !! population of nl orb
         integer                         ::  nprojs=0
...
!        Here we follow the scheme used for the KB projectors
         integer                         ::  n_pjldaunl = 0
          !! num of "nl" projs. Not counting the "m copies"
         integer                         ::  lmax_ldau_projs = 0
          !! l cutoff for LDA+U proj
         integer, dimension(maxn_pjnl)   ::  pjldaunl_l
          !! l of each nl proj
         integer, dimension(maxn_pjnl)   ::  pjldaunl_n
          !! n of each nl proj  
         ...
         real(dp), dimension(maxn_pjnl)  ::  pjldaunl_U
          !! U of each nl projector
         real(dp), dimension(maxn_pjnl)  ::  pjldaunl_J
          !! J of each nl projector
         integer                         ::  nprojsldau = 0
          !! Total number of LDA+U proj.  
          !! Counting the "m copies" (including the `(2l + 1)` factor).
