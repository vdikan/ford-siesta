...
!! author: P.Ordejon, J.M.Soler, and J.Gale
!!
!! Adds the SCF contribution to atomic forces and stress.
!!
!! Written by P.Ordejon, J.M.Soler, and J.Gale.  
!! Last modification by J.M.Soler, October 2000.  
...
!  Passed arguments
      ...
      integer, intent(in) :: ifa
        !! Are forces required? (`1`=yes,`0`=no)
      integer, intent(in) :: istr
        !! Is stress required? (`1`=yes, `0`=no)
      integer, intent(in) :: na
        !! Number of atoms
      integer, intent(in) :: no
        !! Number of basis orbitals
      integer, intent(in) :: nuo
        !! Number of orbitals in unit cell (local)
      integer, intent(in) :: nuotot
        !! Number of orbitals in unit cell (global)
      integer, intent(in) :: np
        !! Number of mesh points (total is nsp*np)
      integer, intent(in) :: nspin
        !! spin%Grid (i.e., `min(nspin,4)`)  
        !! `nspin=1` => Unpolarized,  
        !! `nspin=2` => polarized  
        !! `nspin=4` => Noncollinear spin/SOC
      ...
      integer, intent(in) :: iaorb(no)
        !! Atom to which orbitals belong
      integer, intent(in) :: iphorb(no)
        !! Index of orbital within its atom
      integer, intent(in) :: maxnd
        !! First dimension of Dscf
      integer, intent(in) :: numd(nuo)
        !! Number of nonzero elemts in each row of Dscf
      integer, intent(in) :: listdptr(nuo)
        !! Pointer to start of row in listd
      integer, intent(in) :: listd(maxnd)
        !! List of nonzero elements of Dscf

      real(grid_p), intent(in) :: Vscf(nsp,np,nspin)
        !! Value of SCF potential at the mesh points
      real(grid_p), intent(in) :: Vatm(nsp,np)
        !! Value of Harris potential (Hartree potential
        !! of sum of atomic desities) at mesh points.  
        !! Notice single precision of Vscf and Vatm.

      real(dp), intent(in) ::  dvol
        !! Volume per mesh point
      real(dp), intent(in) ::  VolCel
        !! Unit cell volume
      real(dp), intent(in) ::  Datm(nuotot)
      real(dp), intent(in), target :: Dscf(:,:)
        !! `Dscf(maxnd,*)`: Value of nonzero elemens of density matrix.
        !! It is used in "hermitified" here.

      real(dp), intent(inout) :: Fal(3,*)
        !! Atomic forces (contribution added on output)
      real(dp), intent(inout) :: Stressl(9)
        !! Stress tensor
...
!  Allocate buffers to store partial copies of Dscf and C
...
!  Set logical that determines whether we need to use parallel or serial mode
      Parallel_Run = (Nodes.gt.1)

!  If parallel, allocate temporary storage for Local Dscf
...
!  Find range of a single array to hold force and stress derivatives
!  Range 1-3 for forces
      if (ifa.eq.1) then
        ix1 = 1
      else
        ix1 = 4
      end if
!  Range 4-12 for stress
      if (istr.eq.1) then
        ix2 = 12
      else
        ix2 = 3
...
!  Look for required rows of Dscf not yet stored in D
        do ic = 1,nc
          imp = endpht(ip-1) + ic
          i = lstpht(imp)
...
!  If stress required. Generate stress derivatives
          if (istr.eq.1) then
            do isp = 1,nsp
              ii = 0
              do ix = 1,3
...
!     Factor two for nondiagonal elements for non-collinear spin (and SO)
        if ( nspin == 4 ) then
...
!  Copy force and stress gradients to a single array
          gCi(1:3, 1:nsp) =  gC(1:3,1:nsp,ic)
          gCi(4:12,1:nsp) = xgC(1:9,1:nsp,ic)

!  Find Sum_{j,spin}(Vscf*Dij*Cj) for every subpoint
!  Some loops are not done using f90 form as this
!  leads to much slower execution on machines with stupid f90
...
!  Add 2*Dscf_ij*<Cj|Vscf|gCi> to forces
          do ix = ix1,ix2
            DF(ix) = 0.0_dp
            do isp = 1, nsp
...
!  Add force and stress to output arrays
          if (ifa.eq.1) then
...
!  End of first orbital loop
        enddo

!  End of mesh point loop
      enddo
