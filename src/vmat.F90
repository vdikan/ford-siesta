...
!!
!! Version of vmat that uses a direct algorithm to save memory.
...
! Argument types and dimensions
    ...
    integer, intent(in)      :: nvmax
      !! First dimension of `listV` and `Vs`, and maximum
    ...
    integer, intent(in)      :: numVs(nuo)
      !! Number of non-zero elements in a row of `Vs`
    ...
    integer, intent(in)      :: listVs(nvmax)
      !! List of non-zero elements of `Vs`
    real(grid_p), intent(in) :: V(nsp,np,spin%Grid)
      !! Value of the potential at the mesh points
