...
!!
!!#### Theory
!! As presented in Kresse and Furthmuller
...
!!
!! * [[dhscf_init(proc)]]: the allocation of the rhog arrays is done there once
!! the mesh-related parameters are known.
!! * [[dhscf(proc)]]: It can optionally use rhog_in instead of Dscf as starting
!! point. It also computes and stores rhog by default.
!! * [[compute_energies(proc)]]: Apart from correcting EKS when mixing the charge,
!! it generates rhog from DM_out.
!! * [[siesta_forces(proc)]]: In transiesta runs the history is reset upon starting
!!   transiesta
!! * [[setup_hamiltonian(proc)]]: it saves rhog_in.
